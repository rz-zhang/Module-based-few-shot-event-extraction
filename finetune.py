...
from data_load import ACE2005Dataset, ACE2005DatasetBase, ACE2005DatasetNovel, pad, all_triggers, all_triggers_noBIO, all_entities, all_postags, all_srl, tokenizer
    ...
    for i, batch in enumerate(iterator):
        ...
        if module_mode=="module":
            if len(argument_keys) > 0:
                ...
                # --- Comment this block for the ablation study---
                    # for gating
    ...
    for epoch in range(1, hp.n_epochs + 1):
        ...
        if hp.eval_mode=='gating':
            ...
            if dev_f1 >= dev_f1_max:
                ...
                model_save_path = os.path.join(hp.module_output, 'pretrain_'+hp.model_save_name)
    ...
    for epoch in range(1, hp.ft_epochs + 1):
        ...
        if hp.eval_mode == 'gating':
            ...
            if dev_f1 >= dev_f1_max:
                dev_f1_max = dev_f1
                metric_output = os.path.join(hp.module_output, hp.result_output)
                model_save_path = metric_output + hp.model_save_name
                torch.save(model, model_save_path)
                with open(metric_output, 'a') as fout:
                  fout.write(f"=========eval dev at epoch={epoch}=========\n")
                  fout.write(metric_dev)
